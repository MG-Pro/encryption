...
export class KjbSteganographyService {
  private canvas: HTMLCanvasElement
  private ctx: CanvasRenderingContext2D // храним ссылку на контекст холста
  private signalEnergy = 0.15 // чем выше значение, тем устойчивее к искажениям (+ заметнее)
  private sigmaMin = -3
  private sigmaMax = 3
  private repeat = 20 // количество встраиваний
  ...
  public async encrypt(img: File, message: string, keys: number[]): Promise<string> {
  ...
  public async decrypt(img: File, keys: number[]): Promise<string> {
    return await '';
  }
  ...
  writeMessage(text: string, srcImg: ImageData, keys: number[]) {
    const binaryMessage = this.getBinaryMessage(text)
    ...
    let repeatBinaryMessage = this.getRepeatedMsg(binaryMessage, this.repeat)
    let pxs = this.getPxsForBits(srcPx, this.canvas.width, this.canvas.height, this.sigmaMin, this.sigmaMax, this.signalEnergy, repeatBinaryMessage)

    for (let i = 0; i < pxs.length; i++) {
      brg[i] = this.getPxsBrightness(srcPx, pxs[i], this.signalEnergy)
      modBlue = this.modifyBlueChannel(repeatBinaryMessage[i], srcPx[4 * pxs[i] + 2], this.signalEnergy, brg[i])
...
//  Генерация изображения-ключа для последующего извлечения данных
    ...
    if (this.validate(key1) == 0 && this.validate(key2) == 0 && this.validate(key3) == 0) {
  ...
  readMessage() {
    ...
    let srcImgData = this.ctx.getImageData(0, 0, width, height)
...
// Функция для отбора допустимых для встраивания пикселей
  getPxsForBits(Pixel, Width, Height, SigmaMinValue, SigmaMaxValue, SignalEnergy, Text) {
...
// Функция для определения яркости определённого пикселя
  getPxsBrightness(Pixel, PxNum, SignalEnergy) {
...
// Функция, возвращающая массив значений яркости пикселей
  getArrPxsBr(Pixel, SignalEnergy) {
...
// Функция, возвращающая изменённое значение синего канала
  modifyBlueChannel(msgBit, srcBlueChannel, signalEnergy, PxBrightness) {
...
// Функция возврата массива с прогнозируемыми значениями синего канала
  getForecastBlueChannels(BlueChannel, Width, Height, sigmaMinValue, sigmaMaxValue) {
...
// Восстановление бинарной последовательности
  restoreBinaryMessage(Pixel, PixelsWithData, Width, Height, sigmaMinValue, sigmaMaxValue, Repeat) {
...
// Функция для суммы ряда по горизонтали по заданному диапазону
  getRowSum(PxArray, Width, Height, MinSigmaValue, MaxSigmaValue) {
    let sum = 0
    let sumValues = []
...
// Функция для суммы ряда по вертикали по заданному диапазону
  getColumnSum(PxArray, Width, MinSigmaValue, MaxSigmaValue) {
    let sum = 0
    let sumValues = []
...
// Перевод бинарной последовательности в ASCII
  getTextFromBinary(binaryMessage) {
    let srcMessage = ''
    let add = 0
